      ...
      subroutine read_settings(av,bcs)
                ...
                read(5, '(A)', IOSTAT=ios) tempname
                if (ios /= 0) then
                    write(6,*) 'Error reading casename'
                    stop
                end if
                av%casename = trim(tempname)
                write(6,*) 'Casename:', av%casename

                
                read(5, '(A)', IOSTAT=ios) input_line
                if (ios /= 0) then
                    write(6,*) 'Error reading input line for gam and rgas'
                    stop
                end if
                write(6,*) 'Input line for gam and rgas:', input_line

                ! Read gam and rgas from the input line
                read(input_line, *, IOSTAT=ios) av%gam, av%rgas
                if (ios /= 0) then
                    write(6,*) 'Error reading gam and rgas'
                    stop
                end if
                write(6,*) 'Read settings'
                write(6,*) av%gam, av%rgas
